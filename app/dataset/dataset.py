...
class DatasetHandler(AbstractDatasetHandler):
    ...
    def apply_transforms(batch, dataset_type: str) -> dict:
        ...
        # Mnist Value -> add fashion_mnist (0.1307,), (0.3081,
        pytorch_transforms = Compose([ToTensor(), Normalize(mean, std)])
    ...
    def load_federated_dataset(self):
        ...
        self.fds = FederatedDataset(
            dataset=self.datasetID, partitioners={"train": partitioner})
        self.client_datasets = [self.fds.load_partition(
            user_id) for user_id in range(self.num_clients)]
        self.logger.info("Federated dataset loaded and partitioned.")
        if self.label_flipping_attack:
            # manual setting attackers to be updated
    ...
    def _apply_transforms_to_clients(self):
        ...
        def transform(batch): return DatasetHandler.apply_transforms(
            batch, dataset_type=self.datasetID)
    ...
    def attack_client_datasets_structured(self, attacked_clients: list, flip_percentage: float, num_classes: int = 10, mapping: dict = None):
        ...
        self.logger.info(
            f"Attacking clients {attacked_clients} with {flip_percentage}% structured label flips.")
        for client_id in attacked_clients:
            self.logger.info(
                f"Attacking client {client_id} dataset using structured mapping.")
            ...
            indices_to_flip = set(random.sample(
                range(total_examples), n_to_flip))
            ...
            client_dataset = client_dataset.map(
                lambda example, idx: structured_flip(example, idx), with_indices=True)
            # Update the client dataset with the attacked version
            self.client_datasets[client_id] = client_dataset

        self.logger.info(
            "Structured attack complete on selected client datasets.")

    def server_dataset(self, batch_size: int) -> DataLoader:
        fds = FederatedDataset(dataset=self.datasetID, partitioners={
                               "train": IidPartitioner(num_partitions=600)})
        def transform(batch): return DatasetHandler.apply_transforms(
            batch, dataset_type=self.datasetID)
    ...
    def _calculate_label_distributions(self):
        ...
        for client_id in range(self.num_clients):
            ...
            if hasattr(client_dataset, 'with_format'):
                ...
                labels = [int(client_dataset[i][1].item())
            ...
            self.logger.info(
                f"Client {client_id} label distribution: {label_percentages}")
    ...
    def get_label_distribution_matrix(self) -> np.ndarray:
        ...
        if not self.client_label_distributions:
            self.logger.warning(
                "Label distributions not calculated. Run _calculate_label_distributions() first.")
            return np.array([])
    ...
    def find_optimal_k_kmeans(self, max_k: int = 10, plot: bool = True) -> dict:
        ...
        """
        self.logger.info(
            "Finding optimal K for K-means clustering using silhouette score.")
        ...
        for k in k_range:
            ...
            self.logger.info(
                f"K={k}: Silhouette={sil_score:.3f}, Calinski-Harabasz={cal_score:.3f}, Davies-Bouldin={db_score:.3f}")
    ...
    def perform_clustering_analysis(self, methods: list = ['kmeans', 'dbscan', 'hierarchical'],
        ...
        """
        self.logger.info(
            f"Performing clustering analysis using methods: {methods}")
        ...
        if 'dbscan' in methods:
            ...
            if best_dbscan is not None:
                ...
                self.logger.warning(
                    "DBSCAN could not find valid clustering with tested parameters.")
        ...
        if store_method in results:
            ...
            self.clusters = {client_id: int(
                labels[client_id]) for client_id in range(self.num_clients)}
            self.logger.info(
                f"Stored {store_method} clustering results. Clusters: {self.clusters}")
        else:
            self.logger.warning(
                f"Store method '{store_method}' not found in results. Clusters not stored.")
        ...
        for method, result in results.items():
            ...
            for cluster_id in unique_labels:
                if cluster_id != -1:  # Skip noise points in DBSCAN
                    clients_in_cluster = [i for i, label in enumerate(
                        result['labels']) if label == cluster_id]
...
class DatasetHandlerTab(AbstractDatasetHandler):
    ...
    def transform_client_dataset(dataset: pd.DataFrame) -> pd.DataFrame:
        ...
        if categorical_columns:
            ...
            dataset[categorical_columns] = ordinal_encoder.fit_transform(
        ...
        numeric_features = X_train.select_dtypes(
            include=['float64', 'float32', 'int64']).columns.tolist()
        ...
        y_train_tensor = torch.tensor(
            y_train.values, dtype=torch.long).view(-1, 1)
    ...
    def load_federated_dataset(self):
        ...
        if self.datasetID == 'mstz/covertype':
            self.fds = FederatedDataset(
                dataset=self.datasetID, subset='covertype', partitioners={"train": partitioner})
            ...
            self.fds = CustomFederatedDataset(
                dataset={'train': dataset}, partitioners={"train": partitioner})

        else:
            self.fds = FederatedDataset(
                dataset=self.datasetID, partitioners={"train": partitioner})
        self.client_datasets = [self.fds.load_partition(user_id).with_format('pandas')[
    ...
    def load_test_data(self):
        ...
        """
        self.logger.info(
            "Loading test dataset for tabular data by aggregating all partitions and splitting.")
        # Aggregate the full dataset from all client partitions.

        full_dataset = pd.concat(
            [self.fds.load_partition(user_id).with_format(
                'pandas')[:] for user_id in range(self.num_clients)],
            ignore_index=True
        )
        train_data, test_data = train_test_split(
            full_dataset, test_size=0.2, random_state=self.seed)

        # Process the test split
        test_tensor_dataset = DatasetHandlerTab.transform_client_dataset(
            test_data)

        self.logger.info(
            "Test dataset loaded and transformed for tabular data.")
