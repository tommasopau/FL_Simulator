...
class Server:
    ...
    def _sample_clients(self, num_of_sampled: int) -> Tuple[List[int], int]:
        ...
        sampled_clients = random.sample(
            range(self.num_clients), num_of_sampled)
        ordered_clients, number_of_attackers_epoch = self._order_clients_by_attackers(
            sampled_clients)
    ...
    def _sample_from_weights(self, weights: Dict[int, float], num_of_sampled: int) -> Tuple[List[int], int]:
        ...
        filtered_weights = {client_id: weight for client_id,
                            weight in weights.items() if weight > 0}
        ...
        normalized_weights = {
            client_id: weight / total_weight for client_id, weight in filtered_weights.items()}
    ...
    def _evaluate_global_model(self, epoch: int):
        ...
        with torch.no_grad():
            for batch in self.test_loader:
                ...
                if isinstance(batch, dict):
                    ...
                    raise ValueError(
                        "Unexpected batch format in test_loader. Expected dict with keys ('image','label') or tuple (features, labels).")
        ...
        self.logger.info(
            f"Epoch {epoch} - Test Loss: {avg_loss:.4f}, Test Accuracy: {accuracy:.4f}")
        return accuracy
...
class AttackServer(Server):
    ...
    def run_federated_training(self):
        ...
        for epoch in range(1, self.global_epochs + 1):
            self.logger.info(f"Global Epoch {epoch} started.")
            if self.aggregation_strategy in {AggregationStrategy.KeTS, AggregationStrategy.KeTSV2}:
                sampled_client_ids, number_of_attackers_epoch = self._sample_from_weights(
                    self.trust_scores, self.sampled)
            else:
                sampled_client_ids, number_of_attackers_epoch = self._sample_clients(
                    self.sampled)
            # Use the common training helper
            client_updates = self._train_clients(sampled_client_ids)

            # Extract updates, compute attacked updates, and update client_updates accordingly
            updates_list = [update['flattened_diffs']
                            for (_, update) in client_updates]
            attacked_updates = self._compute_attack(
                updates_list, self.learning_rate,  number_of_attackers_epoch, self.f)
            for i, (cid, update) in enumerate(client_updates):
                client_updates[i] = (
                    cid, {'flattened_diffs': attacked_updates[i], 'data_size': update['data_size']})
...
class FLTrustServer(AttackServer):
    ...
    def _fl_train_server(
        ...
        optimizer = optim.SGD(server_model.parameters(),
        ...
        initial_params = {key: param.clone()
        ...
        for epoch in range(local_epochs):
            ...
            for batch_idx, batch in enumerate(self.server_data_loader):
                if isinstance(batch, dict):
                    ...
                    raise ValueError(
                        "Unexpected batch format. Expected dict with keys ('image','label') or tuple (features, labels).")
        ...
        final_params = {key: param.clone()
                        for key, param in server_model.named_parameters()}
        param_differences = {
            key: (final_params[key] - initial_params[key]) for key in final_params.keys()
        }

        gradient_like_diffs = torch.cat(
            [diff.view(-1) for diff in param_differences.values()]).unsqueeze(1)
    ...
    def _fltrust_aggregation(self, gradients: List[Tuple[int, Dict[str, torch.Tensor]]],
        ...
        for grad in param_list:
            ...
            cosine_sim = torch.dot(baseline, grad_squeezed) / (
                (torch.norm(baseline) + 1e-9) *
                (torch.norm(grad_squeezed) + 1e-9)
            )
        ...
        with torch.no_grad():
            ...
            for param in net.parameters():
                numel = param.numel()
                param.add_(global_update[idx: idx +
                           numel].view(param.size()), alpha=1)
                idx += numel
