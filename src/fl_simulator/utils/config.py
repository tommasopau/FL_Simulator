...
class ModelConfig(BaseModel):
    type: str = Field(...,
                      description="Type of the model. Must be 'MNISTCNN' or 'FCMNIST'.")

    @field_validator('type')
    def validate_model_type(cls, v):
        allowed_types = ['MNISTCNN', 'FCMNIST', 'ZalandoCNN',
                         'ResNetCIFAR10', 'TinyVGG', 'KDDSimpleNN']  # Add more as needed
...
class DatasetConfig(BaseModel):
    ...
    num_clients: int = Field(..., ge=2,
                             description="Number of federated clients. Must be at least 2.")
    alpha: float = Field(..., ge=0.05, le=10.0,
                         description="Alpha value for the partitioning of the dataset.")
    batch_size: int = Field(..., ge=1,
                            description="Batch size for the federated learning.")
    partition_type: PartitionType = Field(...,
                                          description="Type of data partitioning.")


class TrainingConfig(BaseModel):
    global_epochs: int = Field(..., ge=1,
                               description="Number of global epochs for federated learning.")
    local_epochs: int = Field(..., ge=1,
                              description="Number of local epochs for federated learning.")
    learning_rate: float = Field(..., gt=0,
                                 description="Learning rate for the federated learning.")
    sampled_clients: int = Field(..., ge=1,
                                 description="Number of clients sampled in each round.")

    local_DP_SGD: bool = Field(
        False, description="Flag to indicate if local DP-SGD is used.")
    fedprox: bool = Field(
        False, description="Whether to use FedProx algorithm")
    fedprox_mu: float = Field(
        0.01, ge=0, description="FedProx regularization parameter")

    optimizer: str = Field("SGD", description="Optimizer type")
    momentum: float = Field(
        0.9, ge=0, le=1, description="Momentum for SGD optimizer")


class AttackConfig(BaseModel):
    attack: AttackType = Field(
        AttackType.NO_ATTACK, description="Type of attack to be used")
    num_attackers: int = Field(
        0, ge=0, description="Number of attackers in the federated learning.")
...
class Aggregation(BaseModel):
    aggregation_strategy:  AggregationStrategy = Field(
        AggregationStrategy.FEDAVG, description="Strategy for aggregating the models.")
    ...
    def validate_client_constraints(self) -> 'Config':
        ...
        if self.attack.attack != AttackType.NO_ATTACK:
            if self.attack.num_attackers <= 0:
                raise ValueError(
                    "num_attackers must be greater than 0 when an attack is specified")
