...
class SimulationService:
    ...
    def _initialize_model(self):
        ...
        if model_class is None:
            raise ValueError(
                f"Unsupported model type: {self.config.model.type}")
    ...
    def _create_server(self):
        ...
        try:
            ...
            logger.info(
                f"Server configuration: Aggregation={aggregation_strategy}, Attack={attack_type}")
    ...
    def _store_results(self, accuracy: float):
        ...
        try:
            ...
            logger.info(
                f"Simulation result stored - ID: {result.id}, Accuracy: {accuracy:.4f}")
...
def create_simulation_from_dict(config_dict: dict) -> SimulationService:
    ...
    try:
        ...
        if not config_dict:
            raise ValueError(
                "Configuration dictionary cannot be None or empty")
