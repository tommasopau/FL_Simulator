...
def cluster_similarity_fedavg(
    ...
    **kwargs
    ...
    if clusters is None:
        logger.warning("No cluster information provided. Using standard FedAvg.")
    ...
    for cid, gradient in gradients:
        cluster_id = clusters.get(cid, -1)  # Default to -1 if client not in clusters
        cluster_groups[cluster_id].append((cid, gradient))
    
    logger.info(f"Grouped {len(gradients)} clients into {len(cluster_groups)} clusters")
        ...
        for i, (cid1, grad1) in enumerate(cluster_clients):
            ...
            for j, (cid2, grad2) in enumerate(cluster_clients):
                if i < j:  # Only calculate upper triangle to avoid duplicates
                    ...
                    similarity = F.cosine_similarity(flat_grad1, flat_grad2, dim=0).item()
                    
                    # Only log negative similarities
                    if similarity < 0:
                        logger.warning(f"NEGATIVE SIMILARITY in Cluster {cluster_id}: Client {cid1} and Client {cid2}: {similarity:.4f}")
    ...
    if negative_similarity_clients:
        logger.warning(f"Clients with negative cosine similarities: {sorted(negative_similarity_clients)}")
    else:
